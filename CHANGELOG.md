...
split into `mlox_subset/` — 7 subpackages, 33 modules — with
...
not on disk. The suite grew to **802 tests** over the course of the split
...
- **Translation marking, complete.** `mlox_subset/i18n.py` provides gettext
  ...
  (**312 messages**), regenerated by the new **`tools/make_pot.py`**:
...
- **mypy now gates the entire codebase** — all **35** files, up from 28.
...
- **The engine was split into the `mlox_subset/` package.** Seven subpackages
  ...
  invocation and test keeps working unchanged. The package was held to a
...
- **`CODE_REVIEW.md` is now labelled as the running log it always was**, with a
  ...
  split and misses the largest one (`compute_plan`, 545 lines), and deleting the
  re-export shim it recommends is now a breaking change, since 3.0 promises
  `core.<name>` keeps working.

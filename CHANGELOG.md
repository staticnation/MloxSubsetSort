...
split into `mlox_subset/` — 6 subpackages, 28 modules — with
`mlox_subset_sort.py` kept as the compatibility surface that re-exports
everything, so `core.<name>` still resolves for the GUI, the CLI and the tests.
Nothing about the interface changed; what changed is that the pieces are now
separable enough to test and read individually.
...
not on disk. The suite grew to **724 tests** over the course of the split
...
- **Translation groundwork.** `mlox_subset/i18n.py` provides gettext lookup,
  plural handling and language auto-detection (`$MLOX_LANG` overrides), and
  **141 user-facing strings are now marked** for translation — buttons, labels,
  tooltips and dialogs. `locale/mlox_subset_sort.pot` is the extracted English
  template a translator starts from, regenerated by the new
  **`tools/make_pot.py`**: standard-library only, so it works on Windows
  without GNU `xgettext`, and AST-based, so a `_()` inside a docstring is
  correctly not extracted. Marking is deliberately partial — messages built
  with f-strings must first become named-placeholder form
  (`_("Loaded %(count)d files") % {"count": n}`) or the extracted text is the
  already-interpolated string, which is no use to a translator. Roughly 127
  such sites remain, tracked in `locale/README.md`. No language ships yet; with
  no catalogue installed every lookup returns the English source unchanged.
...
- **The engine was split into the `mlox_subset/` package.** Six subpackages
  ...
  `momw.py`/`versions.py`/`tracing.py`/`i18n.py` — with `mlox_subset_sort.py`
  retained as a re-export shim so every existing `core.<name>` call site, CLI
  invocation and test keeps working unchanged. New code in `mlox_subset/` is
  held to a stricter standard than the legacy scripts (full typing, PEP 257
  docstrings, no silent excepts), enforced per-file in `pyproject.toml`.

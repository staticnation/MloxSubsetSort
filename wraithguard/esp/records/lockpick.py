...
class LockpickData:
    ...
    @classmethod
    def load(cls, reader: Reader) -> LockpickData:
...
class Lockpick(Record):
    ...
    @classmethod
    def load(cls, reader: Reader, flags: ObjectFlags) -> Lockpick:
        """Read the lockpick's subrecords."""
        ...
        while not reader.at_end:
            ...
            if tag == b"NAME":
                ...
                expect_size(reader, "LOCK", "LKDT", _LKDT_SIZE)
                self.data = LockpickData.load(reader)
            elif tag == b"SCRI":
    ...
    def save(self, writer: Writer) -> None:
        ...
        put_fixed(writer, b"LKDT", _LKDT_SIZE, self.data)
